...
def chat_api():
    ...
    if role == "faculty":
        context = build_faculty_context(current_user)
        user_role_tag = "faculty"
        extra_rules = """
You are talking to a FACULTY member.
They care mostly about their own timetable, their free slots, and avoiding overload.
Prefer suggestions that keep their day compact and avoid late-evening classes.
"""
    elif role == "admin":
        context = build_admin_context(current_user)
        user_role_tag = "admin"
        extra_rules = "" # No extra rules for admin for now
    else:
        context = "User role is not admin/faculty; limited context."
        user_role_tag = role or "unknown"
        extra_rules = "" # No extra rules for unknown roles

    system_prompt = f"""
You are an AI assistant with permission to execute actions inside a university timetable system.

🔥 MOST IMPORTANT RULE:
You MUST ALWAYS respond ONLY in valid, strict JSON.
NO free text outside JSON. NO explanations outside JSON.
...
RULES FOR FACULTY:
Faculty can ONLY perform:
- reschedule_entry
- update_availability

RULES FOR ACTION FORMAT:

1) create_batch requires:
{{
  "name": "string",
  "section": "string",
  "programme": "string",
...
2) regenerate_timetable requires:
{{
  "semester": number,
  "even_or_odd": "even | odd"
}}

3) reschedule_entry requires:
{{
  "entry_id": number,
...
- Ask follow-up questions inside reply_text
- Set action="none"
- Params = {{}}
...
If the user says: "generate new timetable" or "regenerate timetable for all batches"
You MUST respond with:
{{
  "reply_text": "Regenerating timetable for all batches.",
  "action": "regenerate_timetable",
  "params": {{}}
}}

If the user says: "create a new imba batch of 25 students and section B"
You MUST respond with:
{{
  "reply_text": "Preparing to create new IMBA batch section B with 25 students. I need the semester number to continue.",
  "action": "none",
  "params": {{}}
}}

If the user says: "semester 2" (after the previous prompt)
You MUST respond with:
{{
  "reply_text": "Creating batch IMBA section B for semester 2.",
  "action": "create_batch",
  "params": {{
    "name": "IMBA",
    "section": "B",
...
If the user says: "add new faculty named sora"
You MUST respond with:
{{
  "reply_text": "Creating new faculty named Sora.",
...
"""
    ...
    # Call LLM (we modify call_llm to accept messages list instead of 2 strings)
    raw_response = call_llm(messages_for_llm)

    # Parse JSON safely
    try:
        parsed = json.loads(raw_response)
        reply_text = parsed.get("reply_text", raw_response)
...
@main.route("/api/apply_reschedule", methods=["POST"])
@login_required
def apply_reschedule():
    ...
    if not all([entry_id, new_timeslot_id, new_room_id]):
        return jsonify({"success": False, "message": "Missing parameters."}), 400

    ok, msg = can_reschedule(entry_id, new_timeslot_id, new_room_id)
    if not ok:
    ...
    return jsonify({"success": True, "message": "Reschedule applied."})
